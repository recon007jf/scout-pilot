...
# TASK: JAN 6 PRODUCTION PROTOCOL (V5 - MISSING PLAN NUM FIX)
# CONTEXT: Scout-AI Pilot. Generates "Jan6_Master_HitList.csv"
# FIXES: Relies on "4A" code instead of Plan Number 5xx. Maps SPONS_DFE_PN.
# ADAPTATION: Corrected paths for Artifacts vs Output Directory.

def execute_jan6_production_v5():
    # 1. SETUP WORKSPACE
    ARTIFACT_DIR = "Scout_Data_Artifacts"
    OUTPUT_DIR = "pilot_outputs_2021"
    
    print(f">>> SCOUT PRODUCTION PROTOCOL V5 INITIATED")
    ...
    def pick_col(df, candidates, required=True):
        ...
        if required:
            avail = sorted(list(cols_upper.keys()))[:10]
            raise KeyError(f"CRITICAL SCHEMA ERROR: Could not find any of: {candidates}. Available: {avail}...")
        return None
    ...
    # OPTIONAL: Try to grab SPONS_DFE_PN if standard PLAN_NUM is missing
    ...
    # REQUIRED: Welfare Code (Our new primary filter)
    welfare_code_col = pick_col(f5500, ["TYPE_WELFARE_BNFT_CODE"], required=True)

    # FILTER 1: HEALTH ONLY (Based on Code 4A + Name Exclusion)
    print("   Applying Health Filter (Code '4A' + Regex Exclusion)...")
    ...
    print(f"   Using Provider Location Cols: {prov_city_col}, {prov_state_col}")
    ...
    for office in target_offices:
        ...
        if prov_state_col:
             # Check if col exists in keys (it should)
        ...
        for _, row in firm_hits.iterrows():
            ...
            # Get Plan Num safely
            found_pn = row.get(plan_num_col, "") if plan_num_col else "Unknown"
