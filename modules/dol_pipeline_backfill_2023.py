"""
MODULE: AG_DOL_PIPELINE_BROKER_INTEL
OBJECTIVE: Generate 'Western_Broker_Map_2023.parquet' for Enrichment.
VERSION: 9.0 (Broker-Centric / No Hallucinated Emails)
"""
...
# VERIFIED URLS
URL_MAP_2023 = {
    'BASE': "https://askebsa.dol.gov/FOIA%20Files/2023/Latest/F_5500_2023_Latest.zip",
    'SCH_A': "https://askebsa.dol.gov/FOIA%20Files/2023/Latest/F_SCH_A_2023_Latest.zip",
    'SCH_C': "https://askebsa.dol.gov/FOIA%20Files/2023/Latest/F_SCH_C_2023_Latest.zip"
}

# BROKER-CENTRIC SCHEMA
SCHEMA_CONTRACT = {
    'BASE': {
        'ACK_ID': ['ACK_ID'],
    ...
    'SCH_A': {
        'ACK_ID': ['ACK_ID'],
        'BROKER_NAME': ['INS_CARRIER_NAME', 'INS_BROKER_FIRM_NAME', 'BROKER_NAME'],
        # ADDRESS IS CRITICAL FOR ENRICHMENT (Finding the right office)
        'BROKER_ADDRESS': ['INS_CARRIER_US_ADDRESS1', 'INS_BROKER_US_ADDRESS1', 'BROKER_ADDRESS1'], 
        'BROKER_CITY': ['INS_CARRIER_US_CITY', 'INS_BROKER_US_CITY', 'BROKER_CITY'],
        'BROKER_STATE': ['INS_CARRIER_US_STATE', 'INS_BROKER_US_STATE', 'BROKER_STATE']
    },
    'SCH_C': {
        'ACK_ID': ['ACK_ID'],
...
def download_and_extract(dataset_key, output_folder):
    url = URL_MAP_2023[dataset_key]
    zip_filename = f"raw_{dataset_key}_2023.zip"
    zip_path = os.path.join(output_folder, zip_filename)
    os.makedirs(output_folder, exist_ok=True)
    
    if os.path.exists(zip_path) and os.path.getsize(zip_path) > 50000000:
        print(f"   [SKIP] Found existing large {dataset_key}, skipping download.")
    else:
...
def run_broker_intel():
    print("AG STATUS: STARTING BROKER INTEL MAP (v9.0)")
    
    files = {k: download_and_extract(k, ARTIFACT_DIR) for k in URL_MAP_2023.keys()}
    if not files['BASE']:
    ...
    print("   [OPTIMIZE] Filtering for Top 5000 Target Employers...")
    con.execute(f"""
    ...
    filename = "Western_Broker_Map_2023.parquet"
