...
We are integrating Clay to enrich high-priority leads.
* **Configuration:** We will update `.env` with the `CLAY_WEBHOOK_URL` and `CLAY_API_KEY`.
* **Execution:** We will use `execute_clay_live_fire.py` to send batches of leads to Clay.
* **Verification:** We will verify that the script can load these credentials and successfully initiate a request (or fail gracefully if no leads are ready).
