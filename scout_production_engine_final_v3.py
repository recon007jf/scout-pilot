...
class ScoutEngine:
    ...
    def safe_request(self, method, url, **kwargs):
        ...
        for i in range(retries):
            try:
                ...
                if resp.status_code == 429:
                    ...
                    if retry_after:
                        try:
                            ...
                            print(f"      ⚠️ Rate Limit (429). Unparseable header. Backing off {backoff}s...")
                            wait_time = backoff
                            backoff *= 2 
                    else:
                ...
                print(f"      ⚠️ Transport Error: {e}. Retrying...")
                time.sleep(1)
    ...
    def run_batch(self, input_rows, output_file):
        print(f"🚀 SCOUT ENGINE FINAL v3.1 (Safe Backoff). Budget: {CREDITS_BUDGET_CAP}")
        results = []
        ...
        print(f"🛑 Total Rate Limit Hits (Retried): {self.rate_limit_hits}")
